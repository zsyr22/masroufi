...
export async function getCurrentUserPurchaseById(id: string): Promise<PurchaseDetails | null> {
  const supabase = await createClient();
  const { data, error } = await supabase.from("purchases")
    .select("id,transaction_id,store_id,account_id,category_id,channel,branch_name,purchase_date,subtotal,tax,discount,delivery_fee,total,currency,notes,stores(name,branch),accounts(name),categories(name),purchase_items(id,category_id,name,quantity,unit,package_size,package_unit,unit_price,line_total)")
