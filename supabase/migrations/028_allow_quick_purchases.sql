-- Masroufi migration 028: allow total-only purchases and add items later.
...
declare
  ...
  v_package_size numeric(12,3);
  v_package_unit public.purchase_unit;
begin
  ...
  -- Quick purchase: store only the receipt total now and add products later.
  if jsonb_array_length(p_items) = 0 then
    v_subtotal := round(p_total - coalesce(p_tax, 0) - v_delivery_fee + coalesce(p_discount, 0), 2);
    if v_subtotal < 0 then raise exception 'Invalid quick purchase total'; end if;
  end if;
  ...
  loop
    ...
    v_package_size := nullif(v_item->>'packageSize', '')::numeric;

    if trim(coalesce(v_item->>'name', '')) = '' or v_qty <= 0 or v_unit_price < 0 then
      raise exception 'Invalid purchase item';
    end if;

    if v_package_size is not null and v_package_size <= 0 then
      raise exception 'Invalid package size';
    end if;
  ...
  insert into public.transactions(
    user_id, account_id, category_id, payee_id, type,
    amount, currency, transaction_date, notes
  ) values (
    v_user_id, p_account_id, p_category_id, v_payee_id, 'expense',
    p_total, v_account.currency, p_purchase_date,
    coalesce(nullif(trim(p_notes), ''), case when jsonb_array_length(p_items) = 0 then 'Quick purchase · ' || v_store.name else 'Itemized purchase · ' || v_store.name end)
  ) returning id into v_transaction_id;

  insert into public.purchases(
    user_id, transaction_id, store_id, account_id, category_id,
    purchase_date, currency, subtotal, tax, discount,
    delivery_fee, total, notes, channel, branch_name
  ) values (
    v_user_id, v_transaction_id, p_store_id, p_account_id, p_category_id,
    p_purchase_date, v_account.currency, v_subtotal,
    coalesce(p_tax, 0), coalesce(p_discount, 0), v_delivery_fee,
    p_total, nullif(trim(p_notes), ''), p_channel,
  ...
  loop
    ...
    v_line_total := round(v_qty * v_unit_price, 2);
    v_package_size := nullif(v_item->>'packageSize', '')::numeric;
    v_package_unit := nullif(v_item->>'packageUnit', '')::public.purchase_unit;
    ...
    insert into public.purchase_items(
      ...
      quantity, unit, package_size, package_unit,
...
begin
  ...
  set
    ...
    notes = coalesce(nullif(trim(p_notes), ''), case when jsonb_array_length(p_items) = 0 then 'Quick purchase · ' || v_store.name else 'Itemized purchase · ' || v_store.name end),
    updated_at = now()
